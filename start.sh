...
ATF_CLEAN_UP=("ErrorLog.txt" "sdl.pid" "mobile*.out")
...
kill_sdl() {
  local PIDS=$(ps -ef | grep -e "^$(whoami).*$SDL_PROCESS_NAME" | grep -v grep | awk '{print $2}')
  for PID in $PIDS
  do
    local PID_INFO=$(pstree -sg $PID | head -n 1 | grep -v docker)
    if [ ! -z "$PID_INFO" ]; then
...
copy_logs() {
  local REPORT_DIRS=("SDLLogs*" "ATFLogs*" "TestingReports_*")
  for DIR in ${REPORT_DIRS[@]}; do
    cp `find ${REPORT_PATH}/$DIR -name "*.*"` ${REPORT_PATH_TS_SCRIPT}/
  done
  cp ${ATF_PATH}/ErrorLog.txt ${REPORT_PATH_TS_SCRIPT}/
}
...
process() {
  ...
  if [ $EXT = "txt" ]; then
    while read -r ROW; do
      ...
      run $(echo $ROW | awk '{print $1}') $(cat $TEST_TARGET | egrep -v -c '^;')
    done < "$TEST_TARGET"
  elif [ $EXT = "lua" ]; then
    run $TEST_TARGET 1
  else
