...
--[[ @isSetContains: check whether set contains value
--! @parameters:
--! pTable - set
--! pValue - value
--! @return: boolean which represents whether set contains value
--]]
function m.isSetContains(pTable, pValue)
  if not pTable then return false end
  for val in pairs(pTable) do
    if val == pValue then return true end
  end
  return false
end
