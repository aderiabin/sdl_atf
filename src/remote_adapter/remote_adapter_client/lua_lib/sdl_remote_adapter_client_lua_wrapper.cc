...
int SDLRemoteTestAdapterLuaWrapper::create_SDLRemoteTestAdapter(lua_State *L) {
    ...
    SDLRemoteTestAdapterClient **s = (SDLRemoteTestAdapterClient **)lua_newuserdata(L, sizeof(SDLRemoteTestAdapterClient *));
...
void SDLRemoteTestAdapterLuaWrapper::registerSDLRemoteTestAdapter(lua_State *L) {
    ...
    lua_setfield(L, -2, "__index"); // Setup index lookup to it self for using it as metatable
    // Index -1(top) : SDLRemoteTestAdapter class table
}

SDLRemoteTestAdapterClient *SDLRemoteTestAdapterLuaWrapper::get_instance(lua_State *L) {
    // Index 1 - lua instance
    lua_getfield(L, 1, "__self"); // get __self key from lua instance and put on top
    // Index -1 (top) - user data ( poiner to C++ implementation pointer)
    SDLRemoteTestAdapterClient** user_data = reinterpret_cast<SDLRemoteTestAdapterClient**>(lua_touserdata(L, -1));
...
int SDLRemoteTestAdapterLuaWrapper::lua_set_data(lua_State *L) {
    auto instance = get_instance(L);
...
int SDLRemoteTestAdapterLuaWrapper::lua_get_data(lua_State *L) {
    auto instance = get_instance(L);
    auto data = instance->get_data();
...
int SDLRemoteTestAdapterLuaWrapper::lua_receive(lua_State *L) {
    auto instance = get_instance(L);
    auto name = lua_tostring(L,-1);
    auto data = instance->receive(name);
