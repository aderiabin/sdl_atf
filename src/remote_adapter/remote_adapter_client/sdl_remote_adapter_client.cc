...
int SDLRemoteTestAdapterClient::open(const std::string& name) try {
  std::cout << "Open Mq " << name << " on remote host: ";
  if (connected()) {
...
int SDLRemoteTestAdapterClient::clear() try {
  std::cout << "Close all Mq handled by server on remote host: ";
  if (connected()) {
...
std::pair<std::string, int> SDLRemoteTestAdapterClient::receive(
  ...
  std::cout << "Receive data from Mq " << name << " on remote host: ";
  if (connected()) {
    ...
    std::cout << "SUCCESS\nReceived data: " << received.first << std::endl;
...
int SDLRemoteTestAdapterClient::handleRpcTimeout(rpc::timeout& t) {
  std::cout << "TIMEOUT expired: " << t.what() << std::endl;
